...
async function loadLang(lang) {
    console.debug(`[RouletteRD][i18n] Loading locale "${lang}" ...`);
    try {
        const res = await fetch(`${LOCALES_PATH}/${lang}.json`);
        jsonLang = await res.json();
        console.debug(`[RouletteRD][i18n] Locale "${lang}" loaded successfully!`);
    }
	catch (err) {
        console.error(`[RouletteRD][i18n] Failed to load locale "${lang}":`, err);
    }
}
